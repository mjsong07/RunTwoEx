...
{
    [[ATAppTransferManager sharedTransfer] setTransferViewFrame:frame];
}
...
- (void)setTransferViewFrame:(CGRect)frame
{
    transferViewFrame_ = frame;
    transferView_.frame = frame;
}
...
{
    transferView_ = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 50)];
    ...
    */
    ...
    for (int i = 0; i < 4; i++) {
        CGRect buttonRect = CGRectMake(0, 0, 48, 48);
        CGPoint center = CGPointMake(26 * (i + 1) + 48 * i + 24, 25);
        UIButton *appButton = [UIButton buttonWithType:UIButtonTypeCustom];
        appButton.frame = buttonRect;
        appButton.center = center;
        appButton.tag = i;
        appButton.autoresizingMask =
    ...
    if (CGRectEqualToRect(transferViewFrame_, CGRectZero) == NO) {
        transferView_.frame = transferViewFrame_;
    }
}
