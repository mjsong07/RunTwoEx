...
- (void)chkUpdateAchieve:(NSString*)achieveKey :(double)nowScore{
    double needScore = [SOXConfig getAchieveNum:achieveKey];
...
- (void)calAchieveAll
{
    ...
    double distance = [SOXDBUtil loadInfoReturnDouble:G_KEY_LEADERBOARD_DISTANCE];
    double nowStar = [SOXDBUtil loadInfoReturnDouble:G_KEY_NOW_STAR_CNT];
    int nowLife = [SOXDBGameUtil loadNowLife];
     
    
    if(bestScore> 0 && bestScore <= G_ACHIEVEMENT_SCORE_L1){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L1 :bestScore];
    }
    if(bestScore > G_ACHIEVEMENT_SCORE_L1 && bestScore <= G_ACHIEVEMENT_SCORE_L2){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L1 :bestScore];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L2 :bestScore];
    }
    if(bestScore > G_ACHIEVEMENT_SCORE_L2 && bestScore <= G_ACHIEVEMENT_SCORE_L3){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L1 :bestScore];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L2 :bestScore]; 
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L3 :bestScore];
    }
   if(bestScore > G_ACHIEVEMENT_SCORE_L3 ){//&& bestScore <= G_ACHIEVEMENT_SCORE_L4
       [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L1 :bestScore];
       [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L2 :bestScore];
       [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L3 :bestScore];
       [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_SCORE_L4 :bestScore];
    }
    
    if(distance> 0 && distance <= G_ACHIEVEMENT_DISTANCE_L1){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L1 :distance];
    }
    if(distance > G_ACHIEVEMENT_DISTANCE_L1 && distance <= G_ACHIEVEMENT_DISTANCE_L2){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L1 :distance];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L2 :distance];
    }
     if(distance > G_ACHIEVEMENT_DISTANCE_L2 && distance <= G_ACHIEVEMENT_DISTANCE_L3){ 
         [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L1 :distance];
         [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L2 :distance];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L3 :distance];
    }
     if(distance > G_ACHIEVEMENT_DISTANCE_L3 ){//&& distance <= G_ACHIEVEMENT_DISTANCE_L4
         [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L1 :distance];
         [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L2 :distance];
         [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_DISTANCE_L3 :distance];
...
{
    if(stars > 0 && stars <= G_ACHIEVEMENT_STAR_L1){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L1 :stars];
    }
    if(stars > G_ACHIEVEMENT_STAR_L1 && stars <= G_ACHIEVEMENT_STAR_L2){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L1 :stars];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L2 :stars];  
    }
    if(stars > G_ACHIEVEMENT_STAR_L2 && stars <= G_ACHIEVEMENT_STAR_L3){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L1 :stars];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L2 :stars];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L3 :stars];
    }
    if(stars > G_ACHIEVEMENT_STAR_L3){// && stars <= G_ACHIEVEMENT_STAR_L4
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L1 :stars];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L2 :stars];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L3 :stars];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_STAR_L4 :stars];
    }
}

- (void)calAchieveLife:(int)nowLife
{ 
    if(nowLife > 0 && nowLife <= G_ACHIEVEMENT_LIFE_L1){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L1 :nowLife];
    }
    if(nowLife > G_ACHIEVEMENT_LIFE_L1 && nowLife <= G_ACHIEVEMENT_LIFE_L2){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L1 :nowLife];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L2 :nowLife];
    }
    if(nowLife > G_ACHIEVEMENT_LIFE_L2 && nowLife <= G_ACHIEVEMENT_LIFE_L3){
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L1 :nowLife];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L2 :nowLife];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L3 :nowLife];
    }
    if(nowLife > G_ACHIEVEMENT_LIFE_L3 ){//&& nowLife <= G_ACHIEVEMENT_LIFE_L4
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L1 :nowLife];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L2 :nowLife];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L3 :nowLife];
        [self chkUpdateAchieve:G_KEY_ACHIEVEMENT_LIFE_L4 :nowLife];
    }
}
